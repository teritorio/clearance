      ...
      locha.semantic_clusters.each_with_index.collect{ |cluster, semantic_group_index|
        cluster.links.collect{ |link|
          ValidationLog.new(
            ...
            before_objects: (Osm::ObjectChangeId.from_hash(link.conflation.before.to_h.except('geojson_geometry', 'members', 'uid', 'username', 'username_blocks_received_count', 'osm_user', 'created', 'tags', 'changeset', 'is_change', 'group_ids', 'cc_id'), true) if !link.conflation.before.nil?),
            after_objects: (Osm::ObjectChangeId.from_hash(link.conflation.after.to_h.except('geojson_geometry', 'members', 'uid', 'username', 'username_blocks_received_count', 'osm_user', 'created', 'tags', 'changeset', 'is_change', 'group_ids', 'cc_id'), true) if !link.conflation.after.nil?),
