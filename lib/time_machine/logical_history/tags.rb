    ...
    def self.key_val_main_distance(tags_a, tags_b)
      ...
      keys.sum(0.0) { |key|
        if tags_a[key] == tags_b[key]
          0.0
        elsif tags_a.key?(key) && tags_b.key?(key) && MAIN_TAGS_CLASS_OF_VALUES.key?(key) && MAIN_TAGS_CLASS_OF_VALUES[key].nil? || (MAIN_TAGS_CLASS_OF_VALUES[key]&.include?(tags_a[key]) && T.must(MAIN_TAGS_CLASS_OF_VALUES[key]).include?(tags_b[key]))
